...
def log_metrics(metrics, step):
    ...
    current_step = step  # Update the current step
...
def train_one_batch(
    ...
    for k, v in losses_dict.items():
        log_dict[f"train/actual_full_loss_{k}"] = v.item()
    for k, v in masked_c_losses.items():
...
def _main(hydra_cfg):
    ...
    attention_masks = train_batch.pop("attention_masks", None)
    tokenized = tokenizer_manager.encode(train_batch, attention_masks=attention_masks)
