    ...
    def forward_loss(
        ...
        for key in targets.keys():
            ...
            if discrete_map[key]:
                ...
                # FIXED: Remove target normalization and /100 scaling that was causing artificially low losses
                # if norm == "l2":
                #     target = target / torch.norm(target, dim=-1, keepdim=True)
                raw_loss = nn.MSELoss(reduction="none")(pred, target)
                # raw_loss = nn.MSELoss(reduction="none")(pred, target)/100  # REMOVED problematic /100 scaling
