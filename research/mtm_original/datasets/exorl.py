...
def get_datasets(
    ...
    print("domain: ", domain)
    replay_train_dir = Path(replay_buffer_dir) / domain / "proto" / "buffer"
...
class OfflineReplayBuffer(Dataset, DatasetProtocol):
    ...
    def trajectory_statistics(self) -> Dict[str, DataStatistics]:
        ...
        # Load precomputed statistics if they exist
        if save_path.exists():
            print("Loading precomputed statistics.")
            with open(save_path, "rb") as f:
                ret_dict = pickle.load(f)
                print("checking use qpos condition: ", self._use_qpos)
                if self._use_qpos:
                    ret_dict["states"] = ret_dict["qpos"]
                if self._use_remove_vel:
                    new = DataStatistics(
        ...
        # Find all episode files in the replay buffer directory
        # Update to directly use the correct directory
        print(f"Replay buffer directory: {self._replay_dir}")
        eps_fns = sorted(self._replay_dir.rglob("*.npz"))
        print(f"Found {len(eps_fns)} episode files.")
        for file in eps_fns:
        ...
        if not eps_fns:
            raise FileNotFoundError(
                f"No episode files (*.npz) found in directory: {self._replay_dir}"
            )

        keys = ["states", "actions", "rewards"]
        stats = {key: [] for key in keys}
        print(f"Saving new statistics to: {save_path}")

        # Process each episode
        for eps_fn in tqdm.tqdm(eps_fns):
            episode = load_episode(eps_fn)
            if episode is None:
                print(f"Skipping empty or invalid episode file: {eps_fn}")
                continue
            ...
            stats["actions"].append(episode["action"])
            stats["rewards"].append(episode["reward"])
            if "qpos" in episode.keys() and episode["qpos"] is not None:
                stats["qpos"].append(episode["qpos"])
        ...
        for key in keys:
            if stats[key]:
                ...
                raise ValueError(f"No valid data for key: {key}")
...
def main():
    ...
    env_name = "point_mass_maze_reach_top_left"

    # Load Hydra configuration dynamically
    config_path = "path/to/hydra/config.yaml"  # Replace with the actual path
    config = OmegaConf.load(config_path)

    # Get replay buffer directory from the configuration
    replay_buffer_dir = Path(config.dataset.replay_buffer_dir)

    # Create the environment using the specified name and seed
    env = dmc.make(env_name, seed=0)

    # Resolve the domain dynamically based on the environment name
    if env_name.startswith("point_mass_maze"):
        domain = "point_mass_maze"
    else:
        domain = env_name.split("_", 1)[0]
    ...
    # Construct the replay buffer training directory
    replay_train_dir = replay_buffer_dir / domain / "proto" / "buffer"
